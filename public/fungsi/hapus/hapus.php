...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty(htmlentities($_GET['kategori']))) {
        ...
        $row -> execute($data);
        echo '<script>window.location="../../dashboard.php?page=kategori&&remove=hapus-data"</script>';
    }
    ...
    if (!empty(htmlentities($_GET['barang']))) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../dashboard.php?page=barang&&remove=hapus-data"</script>';
    }
    ...
    if (!empty(htmlentities($_GET['jual']))) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../dashboard.php?page=jual"</script>';
    }
